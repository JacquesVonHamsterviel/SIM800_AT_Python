...
common_input_function=True #通用输入功能，通过运行目录下的common_input.txt 实现，仅读取第一行的命令，超出部分会删除，流程完毕会返回free在第一行，或者error和第二行错误信息。
...
while True:
    try:
        ...
        if res!=b'' and res!=b'\r\n':
             ...
             if res==b'\\x00':
                log("LOG","检测到\\x00，重新连接串口")
                ser.close()
                time.sleep(3)
                ser=serial.Serial(serialPort,baudRate,timeout=0.5) 
                log("LOG","检测到\\x00，重新连接串口完成")
             if b'+CMTI:' in res:
                 log("LOG","有短信来了")
